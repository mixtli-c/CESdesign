...
#########################################################################################
### Background file loading for analisis                                              ###
background = np.load(back_filename)

#########################################################################################
### Reference interpolation with the background wavelengths                           ###
no2reference = cf.ref_interpolate(no2_refname,background)
chochoref = cf.ref_interpolate(chocho_refname,background)
...
for n in range(samples):        # Sample loop
    ...
    ax1.plot(wavelengths[minwave:maxwave],
            a+b*fl+no2ref[:,1]*ndensity1+glyref[:,1]*ndensity2,'-r')
