...
ax2 = fig.add_subplot(212)      # Axes 2 : Concentration
...
for n in range(samples):        # Sample loop
    ...
    ### This one does everything (see recursive_fit function in CESfunctions.py)
    ...
    ax2.plot(meastime2[:n+1],ppbs[:n+1])
...
np.savetxt(path_file + "M" + t1.strftime('%y%m%d%H%M') + '.txt',
        np.column_stack((meastime,ppbs,area)), fmt='%s')
