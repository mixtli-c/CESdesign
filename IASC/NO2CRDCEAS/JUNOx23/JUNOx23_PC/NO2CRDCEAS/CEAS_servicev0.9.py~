...
def ceas_blank(fig,ax1,ax2,meastype,accums,exposure,shots):
    ...
    meastype = ['b','z']
    '''
    global xpixels,path_file,measures
    ...
    measurements = measures = np.copy(wavelengths).reshape(len(wavelengths),1)
...
def ceas_measure(fig,ax3,ax4,meastype,accums,exposure,shots,mfca_p,mfca_t,trueref):
    ...
    NEEDS WORK!
    '''
    global  meastime,meastime2,ppbs,mfca_ps,mfca_ts,flag,path_file,xpixels,measures
    ...
    for i in range(meas_shots):
        ...
        np.savetxt(path_file+'Mtemp.txt',np.column_stack(
            (meastime,ppbs,mfca_ps,mfca_ts,flag)),fmt='%s')
...
while True:
    try:
        ...
        choice = input('R - Reload conf file. C - Change temp. Otherwise will end:')
        if choice.lower() == 'r':
            import configurations as conf
            continue
        elif choice.lower() == 'c':
