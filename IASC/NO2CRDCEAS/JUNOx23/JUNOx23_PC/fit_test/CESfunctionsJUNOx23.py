...
fit_signal_w_fl(extinction,f_l,reference1,reference2)
...
get_fl_2ref(I_0,I_sample,reference1,reference2,density1,density2,Reff,distance,
        npoints=17,npoly=3)
get_fl_broad(alpha,reference1,reference2,density1,density2,npoints=51,npoly=8)
...
def fit_alg_1A(I_sample,I_0,Reff,distance,reference1,pPa,tK,verbose=0,parameters=0,choice=2):
    ...
    if c < 0:
        c=0
    alpha = alpha.reshape(len(alpha),1)
    fl = get_fl_broadA(alpha,reference1,c,npoints=51,npoly=8)
