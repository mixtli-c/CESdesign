            ...
            while crdsloop:
                ...
                for m in range(cyclescrds):
                    ...
                    np.savetxt(path_file + 'MCtemp.txt',
                                np.column_stack((meastime,ppbs,taus,pPSI,tC,mfca_mf,mfcf_mf,flag)),
                               fmt='%s')
                    ...
                    np.savetxt(path_file + 'MCtemp.txt',
                                np.column_stack((meastime,ppbs,taus,pPSI,tC,mfca_mf,mfcf_mf,flag)), fmt='%s')
        ...
        case 'c':
            ...
            while ceasloop:
                ...
                for i in range(cycles):
                    ...
                    try:
                        trueref = Reff_scale[-1] 
                    except:
                        trueref = '0.99992'

                    for j in range(int(shots_meas)):
                        msg_out = 'm' + ',' + accums + ',' + exposure + ',1,' + mfcadata+','+str(trueref)
                        ceascom.write(bytes(msg_out,'utf-8'))
                        time.sleep(ceasping)
                        msg_in = ceascom.read(ceascom.in_waiting).decode('utf-8')
                        com_count = 0
