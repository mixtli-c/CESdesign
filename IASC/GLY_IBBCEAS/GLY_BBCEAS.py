...
glyreference = np.load(chocho_refname)
...
def run_sample(back_filename,xpixels,accum_number,sdk,no3reference,
    ...
    while True:
        ...
        glyref = np.copy(glyreference[minwave:maxwave,:])
        ...
        try:
            alpha,fl,a,b,ndensity1,ndensity2 = cf.fit_alg_1B_it(I_sample, I_0, Reff,
                    distance,no2ref,glyref,pPa,tK,parameters=1)
        ...
        np.save("ax2adata", np.column_stack((no2ref[:,0],
            a+b*fl+no2ref[:,1]*ndensity1+glyref[:,1]*ndensity2)))
