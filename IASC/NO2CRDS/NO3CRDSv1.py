...
#
# DAQ and Oscilloscope modes available, but the instrument needs to be in the
# corresponding configuration
#
# Written by Dr. Mixtli Campos in August 2024. Adapted from NO2CRDCEASv0.95.py
##########################################################################################

########## General libraries and appending path
import time,sys,serial,os,subprocess
...
pulsehigh = 1e-3   # Pulse high time in seconds
pulselow = 1e-3   # Pulse low time in seconds
pwchannelStart = 0
pwchannelCount = 1
    ...
    match option.lower():
        case 'a':
            ...
            match scenario.lower():
                ...
                case 'z':
                    ...
                    msgz2 = '(#Background,#Zero,#Sample,#Cycles):'
                    msgz=msgz1+msgz2
                    conditions = input(msgz)
                    params = conditions.split(',')
                    accums = params[0]
                    exposure = params[1]
                    shots_bckg = int(params[0])
                    shots_zero = int(params[1])
                    shots_meas = int(params[2])
                    shots_bckg_crds = int(params[0])
                    shots_zero_crds = int(params[1])
                    shots_meas_crds = int(params[2])
                    cycles = int(params[3])
                    cyclescrds = int(params[3])
                    print('Custom conditions:', shots_bckg,shots_zero,
                          shots_meas,cyclescrds)
                case _:
        ...
        case 'b':
            ...
            while crdsloop:
                ...
                ret = instantDo.writeAny(dostartPort, doportCount, 
                                         [shutter+zeroair])
                statusDO = 'DO set to ' + f'{shutter+zeroair:08b}'
                time.sleep(1)
                ...
                pid = subprocess.Popen(["python","NO2CRDS_plotter.py"]).pid
                ...
                for n in range(shots_bckg_crds):
                    ...
                    #scan = AdvPollingOneBufferedAI_TDtp()    
                    #dataset = scan[:x_len]
                    np.save('ax5data',np.column_stack((xs,dataset)))
                    #print(c)
                t1 = dt.datetime.now()
                ...
                for m in range(cyclescrds):
                    ...
                    ret = instantDo.writeAny(dostartPort, doportCount, 
                                             [zeroair])
                    statusDO = 'DO set to ' + f'{zeroair:08b}'
                    print(statusDO)
                    time.sleep(5)
                    ys = [0]*x_len
                    taus_zero = []
                    ...
                    for n in range(shots_zero_crds):
                        ...
                        np.save("ax1data",np.column_stack((xs,dataset)))
                        np.save("ax1adata",np.column_stack((xs,fit)))
                        np.save("ax2data",np.column_stack((xs,dataset-fit)))

                        np.save("ax6data",np.column_stack((meastime2,ppbs)))
                    ...
                    for n in range(shots_meas_crds):
                        ...
                        delta_ext = (2.988-27/1.38066e-17)*((pPa/tK)-(pzPa/tzK))
                        print(delta_ext)
                        print(ext)
                        ppb = ((ext-delta_ext)/2e-17)*(1e15*1.380649e-23*tK/pPa)
                        ppbs.append(ppb)
                        pPSI.append(pres)
                        tC.append(tem)
                        taus.append(tau)
                        ...
                        np.save("ax3data",np.column_stack((xs,dataset)))
                        np.save("ax3adata",np.column_stack((xs,fit)))
                        np.save("ax4data",np.column_stack((xs,dataset-fit)))
        ...
        case 'c':
            ...
            while lccrdsloop:
                print('Switching valves for background measurement (Shutter ON):')
                ret = instantDo.writeAny(dostartPort, doportCount, 
                                         [shutter+sampleclose+zeroair])
                statusDO = 'DO set to ' + f'{shutter+sampleclose+zeroair:08b}'
                print(statusDO)
                time.sleep(1)
                #input('Press any key to collect background.')
                ...
                for n in range(shots_bckg):
                    ...
                    np.save('ax5data',np.column_stack((xs,waveform)))
                ...
                for m in range(cycles):
                    ...
                    pres_zero_mat = np.array(pres_zero)
                    tem_zero_mat = np.array(tem_zero)
                    taus_zero_avg = np.average(taus_zero_mat[5:-4])
                    pres_zero_avg = np.average(pres_zero_mat[5:-4])
                    tem_zero_avg = np.average(tem_zero_mat[5:-4])
                    Reff_scale.append(1-(47/(taus_zero_avg*29979.2458)))
                    ...
                    for n in range(shots_meas):
                        ...
                        pzPa = 6894.757293 * pres_zero_avg
                        tzK = 273.15 + tem_zero_avg
                        delta_ext = (2.988e-27/1.38066e-17)*((pPa/tK)-(pzPa/tzK))
                        #print(delta_ext)
                        #print(ext)
                        ppb = ((ext-delta_ext)/2e-17)*(1e15*1.380649e-23*tK/pPa)
                        ppbs.append(ppb)
                        ...
                        meastime2.append(timenow)
                        print('TAU is: %.2f' %tau)
                        print('NO3 ppb: %.2f' %ppb)
                        print(ppb)
                        
                        np.save("ax3data",np.column_stack((xs,waveform)))
                        np.save("ax3adata",np.column_stack((xs,fit)))
                        np.save("ax4data",np.column_stack((xs,waveform-fit)))
