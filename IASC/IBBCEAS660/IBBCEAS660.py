...
#########################################################################################
#
# The IASC's NO3_IBBCEAS has an Andor iDus camera that connects via a CCI-001 PCI card, 
# this program controls the camera for caputure. Since there is only one accumulation
# at 10s exposure, there is no need to sum signals, the native wait function is enough
#
# Python packages
import os
import subprocess
...
# Local
import configurations_CEAS660 as conf
...
def run_sample(back_filename,xpixels,accum_number,sdk,reference1,reference2,reference3,
    ...
    while True:
        try:
            ...
            np.column_stack((meastime,ppbs,ppbs2,ppbs3)), fmt='%s')
            break
        ...
        ref1 = np.copy(reference1) ### homemade spectrum, 2 columns
        ref2 = np.copy(reference2) ### homemade spectrum, 2 columns
        ref3 = np.copy(np.column_stack((ref1[:,0],reference3))) ### homemade spectrum (1 col)
        ...
        ### This one does everything (see recursive_fit_3ref function in CESfunctions.py)
        try:
            alpha,fl,a,b,ndensity1,ndensity2,ndensity3 = cf.fit_alg_1C_it(I_sample, I_0, Reff,
                    distance,ref1,ref2,ref3,pPa,tK,sigma_ray,parameters=1)
        except Exception as e:
            print("fit_alg_1C_it failed with exception:")
            print(e)
            pass
        ...
        np.savetxt(path_file+'Mtemp.txt',np.column_stack((meastime,ppbs,ppbs2,ppbs3)),fmt='%s')
        ...
        np.save("ax2adata", np.column_stack((ref1[:,0],
            a+b*fl+ref1[:,1]*ndensity1+ref2[:,1]*ndensity2+ref3[:,1]*ndensity3)))
...
while True:
    try:
        ...
        if choice.lower() == 'f':
            pid = subprocess.Popen(["python","IBBCEAS660_plotter.py"]).pid
